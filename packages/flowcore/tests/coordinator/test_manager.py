...
from flowcore.engine.exceptions.plugin import FatalPluginError, RecoverablePluginError
...
def test_linear_graph_orchestration():
    ...
    coord.on_step_completed(task_a)
...
def test_diamond_dag_orchestration():
    ...
    coord.on_step_started("B")
    coord.on_step_completed(task1 if task1.step_id == "B" else task2)
    assert coord.get_next_task() is None
    assert coord.get_step_state("D") == ExecutionState.PENDING
    
    # Complete C, D should unblock
    coord.on_step_started("C")
    coord.on_step_completed(task2 if task2.step_id == "C" else task1)
